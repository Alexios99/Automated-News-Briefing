...
def generate_fund_performance_section(fund_data_path: str) -> str:
    ...
    try:
        ...
        return {
            'best_performers': best_performers.to_dict('records'),
...
def generate_html(briefing: Dict, logo_path) -> str:
    ...
    lines = []
    lines.append(f"<html><head><title>COMPANY Weekly Market Briefing – {str(briefing.get('date', ''))}</title>{style}</head><body>")
    ...
    lines.append('</div>')
    lines.append('</div>')
    lines.append('<div class="briefing-title">Weekly Market Briefing</div>')
